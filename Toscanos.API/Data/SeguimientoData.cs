...
namespace Toscanos.API.Data
{
    public class Seguimiento
    {
        ...
        public int? ObtenerDistritoId(string distrito)
        {
            ...
            var Distrito =  _context.Distritos.Where(x=>x.distrito == distrito).SingleOrDefault();
            if(Distrito != null) 
        ...
        {
            var Provincia =  _context.Provincias.Where(x=>x.provincia == provincia).SingleOrDefault();
            if(Provincia != null) 
        ...
        {
            var Provincia =  _context.Provincias.Where(x=>x.idprovincia == ProvinciaId).SingleOrDefault();
            return Provincia.provincia;
           
            
        }
        public async Task<IEnumerable<GetOrdenTransporte>> Listar_OrdensTransporte(int? remitente_id, int? estado_id
        , int usuario_id,string fec_ini,string fec_fin) 
        {
            return await _repo_Seguimiento.GetAllOrdenTransporte( remitente_id,  estado_id, usuario_id, fec_ini, fec_fin);
        }
        ...
        public async Task<IEnumerable<GetOrdenTransporte>> Listar_OrdensTransporte(int manifiestoId) 
        {
        ...
        public List<Manifiesto> ObtenerEntidades_Manifiesto(List<CargaMasivaDetalle> detalles_cargados )
        {
                ...
                foreach (var vehiculo_cargado in primer_caso)
                {
                    ...
                    var preots = vehiculo_cargado.GroupBy(x=>x.direccion_entrega).ToList();
                     ...
                     foreach (var shipments in preots)
                     {
                         ...
                         foreach (var ot in ots)
                         {
                               ...
                               // String compare = "DISTRIBUCION LOCAL";
                                ...
                                orden.costo = ot.First().costo;
                                orden.valorizado = ot.First().valorizado;
                ...
                // foreach (var item in detalles_cargados)
                // {
                //     //  if(cargados.Where(x=>x.shipment == item.shipment).SingleOrDefault() != null)
                //     //     continue;
                ...
                //      var mismo_tracto = detalles_cargados.Where(x=>x.tracto == item.tracto).ToList(); 
                //      var mismo_remitente =  mismo_tracto.Where(x=>x.remitente == item.remitente).ToList(); 



                
                     
                //     //Generar un manifiesto y varias ots
                //     manifiesto = new Manifiesto();
                //     manifiesto.fecha_registro = DateTime.Now;
                //     manifiesto.numero_manifiesto = "";
                //     manifiesto.usuario_id = 1;
                ...
                //     foreach (var ot in mismo_remitente)
                //     {
